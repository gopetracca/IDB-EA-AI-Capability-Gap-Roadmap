...
> Supporting record for **ADR-0001 · Two scales, never merged**.
> ADR-0001 states the rule. This states where the rule came from, what is borrowed, what is ours,
> and where a reviewer will push. It also answers *"is this standard?"*.
>
> Previously `decisions/two-scales-provenance.md`.

**Status:** Accepted · Version 1.0 · 4 September 2026
...
| Derivation type (ADR-0010) | **Adopted** | **Synthesized** |
...
ADR-0010 supplies the reason to prefer the honest label: *assembled by us is a stronger position with
...
**It is a maturity model, despite the filing.** `analysis/readiness-levels-comparators.md` filed it as
...
`analysis/capability-model-comparison.md` §3.1: *a capability you cannot assign is a
...
A 0–5 structural scale, read off by inspection rather than assessed. From ADR-0002, **two questions
settle every row**:
...
pleasant to report. This is why ADR-0001 ends **both numbers or neither**.
...
informative — it is not well-formed against the object model (ADR-0004, ADR-0005).
...
| Consumption model is recorded, not argued | Field on the realization (ADR-0003) | `model/realization.json` |
...
- Column M is **locked and formula-driven** precisely so nobody can type a readiness value into
  the capability catalog. The ADR-0001 rule is enforced by the workbook, not by discipline.
...
is our construction, recorded as Synthesized under ADR-0010, defined by two questions any reviewer can
apply. The alternative was to cite TRL for a scale that measures something TRL does not measure —
the exact failure mode ADR-0010 exists to prevent.
...
- **Readiness has thinner grounding than the taxonomy.** Every L2 carries named sources; the
  ...
  `analysis/readiness-levels-comparators.md` is where they will point. The answer is §3.1 — the search was done
...
- **Settled by inspection, not by preference.** An earlier draft offered a choice: defend
  ...
  **That second option does not exist** — the slide is a maturity model. We defend readiness as
  synthesized-by-design, which is what ADR-0010 prescribes regardless.

- **Do not let the analyst crosswalk carry this.** `notes/external-model-screenshots.md` shows five of
...
- **ADR-0001** (the rule), **ADR-0002** (the readiness levels),
  **ADR-0005** (two registers), **ADR-0010** (evidence grades and derivation types)
- `analysis/readiness-levels-comparators.md` — the level table, the evidence that establishes each, and the
  corrected World Bank filing
- `analysis/como-funciona-el-modelo.md` — §1, §6 and §7: the same argument for a
  non-specialist audience, with the worked example
- `analysis/capability-model-comparison.md` — §3.1, the ownership argument against
